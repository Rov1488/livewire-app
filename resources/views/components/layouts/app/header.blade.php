...
<!--Header-->
<flux:header container class="border-b mb-1 border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
    ...
    <flux:sidebar.toggle class="lg:hidden" icon="bars-2" inset="left"/>
    <div class="right-0 fixed mr-2.5">
        <!--Multi Languages selector-->
       {{-- <form action="{{route('change.locale')}}" method="get">
            @csrf
            <flux:select wire:model="industry" placeholder="Choose industry...">
                @foreach($languages as $lang)
                    <flux:select.option value="{{ $lang }}" name="locale" > {{$lang}} </flux:select.option>
                @endforeach
            </flux:select>
        </form>--}}
